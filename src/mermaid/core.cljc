(ns mermaid.core
  "Mermaid flowcharts as data — 'hiccup for diagrams'. Mermaid is a concise text diagram language, so a
   ...
   like a scene. A diagram sibling to kotoba.dot (Markdown/web-native rather than GraphViz). `.cljc`.

   A flowchart is a statement list (not infix), so no kotoba.expr. Statements:
